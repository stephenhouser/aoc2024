...
struct charmap_t {
	...
	charmap_t(size_t size_x, size_t size_y) {
	...
	bool is_not_char(int x, int y, char c) const {
		return is_valid(x, y) && !is_char(x, y, c);
	}

	bool is_not_char(size_t x, size_t y, char c) const {
		return is_valid(x, y) && !is_char(x, y, c);
	}
